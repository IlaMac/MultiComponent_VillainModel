...
void Cylinder::draw (
  ...
  const auto model = glm::translate(glm::mat4{1}, p1)
                   * rotationMat
                   * glm::scale(glm::mat4{1}, glm::vec3{radius, radius, glm::distance(p1, p2)});
  ...
  // draw
  glDrawElements(GL_TRIANGLES, 2 * 3 * this->numPhis + 6 * this->numStacks * this->numPhis, GL_UNSIGNED_INT, 0);
