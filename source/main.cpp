...
void mainloop(const std::vector<Node> &Site, struct MC_parameters &MCp, struct H_parameters &Hp, double &my_beta, int &my_ind, struct PT_parameters PTp, struct PTroot_parameters PTroot, const std::string& directory_parameters_temp, int NSTART) {
    ...
    H5Tinsert(MY_HDF5_MEASURES_TYPE, "B_x", HOFFSET(Measures, B_x), H5T_NATIVE_DOUBLE);
    H5Tinsert(MY_HDF5_MEASURES_TYPE, "B_y", HOFFSET(Measures, B_y), H5T_NATIVE_DOUBLE);
    H5Tinsert(MY_HDF5_MEASURES_TYPE, "B_z", HOFFSET(Measures, B_z), H5T_NATIVE_DOUBLE);
    ...
    for (n = NSTART; n<MCp.nmisu; n++) {
        ...
        t_measures.tic();
        mis.reset();
        energies(mis, vil, E_betanp, E_betanm, Site, Hp);
